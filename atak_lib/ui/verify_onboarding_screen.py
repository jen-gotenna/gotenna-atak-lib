"""verify_onboarding_screen -- registers the onboarding-home check.

The locators, expected strings, build-gating, and metadata all live in the YAML
spec (atak_lib/commands/ui/verify_onboarding_screen.yaml) -- the single source of truth.
...
Source of truth: TestRail C147673 "Validate components of the Onboarding home
screen" (Common Functionality Between Apps; plugin 3.0, ATAK variant). See the
YAML for components, the build-gated (PRO-318 / PRO-330) options, the Terms of
Use vs Terms of Service discrepancy, and known gaps.
...
ONBOARDING_LOCATORS = SPEC.locators
...
@command("ui.verify_onboarding_screen", layer="ui",
         summary="Validate components of the onboarding home screen (C147673).")
def verify_onboarding_screen(
    driver,
    *,
    stub: bool = False,
